...
import '../stylesheets/PopularMovies.css';
...
function PopularMovies(){
    ...
    /**fetch movies data from api */ 
      const fetchTrendingMovies = async () =>{
        const {data : {results}} = await Axios.get(`${url}/trending/all/day`,{
            params: {
    ...
    return(
        <>
        <div className="movieListPopular">
            <div className="listTitlePopular">Popular<ArrowForwardIosOutlinedIcon fontSize="small" className="arrow"/></div>
            <div className="wrapperPopular">
                <ArrowBackIosOutlinedIcon className="sliderArrowPopular left" onClick ={()=>handleClick("left")} style={{display: !isMoved && "none"}}/>
                     <div className="movieContainerPopular" ref = {listRef}>
                        {trendingMovies.map(mov =>(
                            <div className="trending" onClick={handleClickPlay}>      
                            <div className="listItemPopular" onMouseEnter={()=>selectMovie(mov)}>
                                <div className="holderPopular">
                                    <img className ='trendingImagePopular' src={`${image_path}${mov.backdrop_path}`}alt ="pic"/>
                                    <p>{mov.title}</p>
                                </div>
                                <div className='over-layPopular'>
